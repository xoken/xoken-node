...
## 0.18.4
### Changed
- Bump Xoken Node to fix peers not stored and excessively verbose logging.
...
### Changed
- Bump `xoken-core` and `xoken-node`.

## 0.13.1
### Changed
- Bump `xoken-node` in `stack.yaml`.
...
## 0.6.3
### Changed
- Dummy release to bump xoken-node in stack.yaml.
...
### Changed
- New versions of NQE and Xoken Node upstream.
...
### Changed
- Split out of former `xoken` repository.
- Use hpack and `package.yaml`.

### Removed
- Removed Stylish Haskell configuration file.
- Remvoed `xoken-core` and `xoken-wallet` packages from this repository.
