...
initDB :: (MonadLogger m, MonadError ImportException m) => Network -> m ()
initDB net = undefined
    -- isInitialized >>= \case
    --     Left e -> do
    --         $(logErrorS) "BlockLogic" $ "Initialization exception: " <> fromString (show e)
    --         throwError (InitException e)
    --     Right True -> do
    --         $(logDebugS) "BlockLogic" "Database is already initialized"
    --         return ()
    --     Right False -> do
    --         $(logDebugS) "BlockLogic" "Initializing database by importing genesis block"
    --         importBlock net (genesisBlock net) (genesisNode net)
    --         setInit

-- getOldOrphans :: (Monad m) => UnixTime -> ConduitT () TxHash m ()
-- getOldOrphans now = getOrphans .| filterC ((< now - 600) . fst) .| mapC (txHash . snd)
importOrphan :: (MonadLogger m, MonadError ImportException m) => Network -> UnixTime -> Tx -> m ()
importOrphan net t tx = do
    $(logDebugS) "Block" $ "Attempting to import orphan tx " <> txHashToHex (txHash tx)
    -- go `catchError` ex
  -- where
  --   go = do
  --       newMempoolTx net tx t >>= \case
  --           True -> $(logDebugS) "BlockLogic" $ "Succesfully imported orphan transaction: " <> txHashToHex (txHash tx)
  --           False -> $(logDebugS) "BlockLogic" $ "Orphan transaction already imported: " <> txHashToHex (txHash tx)
  --       deleteOrphanTx (txHash tx)
  --   ex (OrphanTx _) = $(logDebugS) "BlockLogic" $ "Transaction still orphan: " <> txHashToHex (txHash tx)
  --   ex e = do
  --       $(logErrorS) "BlockLogic" $ "Error importing orphan tx: " <> txHashToHex (txHash tx) <> ": " <> cs (show e)
  --       deleteOrphanTx (txHash tx)

newMempoolTx :: (MonadLogger m, MonadError ImportException m) => Network -> Tx -> UnixTime -> m ()
newMempoolTx net tx w = do
    undefined
  --   $(logInfoS) "BlockLogic" $ "Adding transaction to mempool: " <> txHashToHex (txHash tx)
  --   liftIO $
  --       getTxData (txHash tx) >>= \case
  --           Just x
  --               | not (txDataDeleted x) -> do
  --                   $(logWarnS) "BlockLogic" $ "Transaction already exists: " <> txHashToHex (txHash tx)
  --                   return False
  --           _ -> go
  -- where
  --   go = do
  --       orp <- any isNothing <$> mapM (getTxData . outPointHash . prevOutput) (txIn tx)
  --       if orp
  --           then do
  --               $(logWarnS) "BlockLogic" $ "Transaction is orphan: " <> txHashToHex (txHash tx)
  --           else f
  --   f = do
  --       us <-
  ...
  --           else g ds
  --   g ds = do
  --       $(logWarnS) "BlockLogic" $ "Transaction inputs already spent: " <> txHashToHex (txHash tx)
  --       rbf <-
  ...
  --           else n
  --   r ds = do
  --       $(logWarnS) "BlockLogic" $ "Replacting RBF transaction with: " <> txHashToHex (txHash tx)
  --       forM_ ds (deleteTx net True)
  --       importTx net (MemRef w) w tx
  --       return True
  --   n = do
  --       $(logWarnS) "BlockLogic" $ "Inserting transaction with deleted flag: " <> txHashToHex (txHash tx)
  --       insertDeletedMempoolTx tx w
  --       return False
  --   isrbf th = transactionRBF <$> getImportTx th

revertBlock :: (MonadLogger m, MonadError ImportException m) => Network -> BlockHash -> m ()
revertBlock net bh = do
    ...
    --                             "Attempted to delete block that isn't best: " <> blockHashToHex h
...
importBlock :: (MonadLogger m, MonadError ImportException m) => Network -> Block -> BlockNode -> m ()
importBlock net b n = do
    undefined
    -- mp <- runConduit $ getMempool .| mapC snd .| filterC (`elem` bths) .| sinkList
    -- getBestBlock >>= \case
    --     Nothing
    --         | isGenesis n -> do
    --             $(logInfoS) "BlockLogic" $ "Importing genesis block: " <> blockHashToHex (headerHash (nodeHeader n))
    --             return ()
    --         | otherwise -> do
    --             $(logErrorS) "BlockLogic" $
    ...
    --         | otherwise -> do
    --             $(logErrorS) "BlockLogic" $
    --                 "Block " <> blockHashToHex (headerHash (blockHeader b)) <> " does not build on current best " <>
    --                 blockHashToHex h
    --             throwError (PrevBlockNotBest (blockHashToHex (prevBlock (nodeHeader n))))
  --   insertBlock
  --       BlockData
  ...
  --   $(logDebugS) "Block" $ "Done importing transactions for block " <> blockHashToHex (headerHash (nodeHeader n))
  -- where
  ...
  --                    Nothing -> do
  --                        $(logErrorS) "Block" "Cannot get data for transaction in mempool"
  --                        throwError $ TxNotFound (txHashToHex (txHash tx))
  --           else importTx net (br x) (fromIntegral (blockTimestamp (nodeHeader n))) tx
  ...
  --   w =
  --       let s = B.length (encode b {blockTxns = map (\t -> t {txWitness = []}) (blockTxns b)})
  --           x = B.length (encode b)
  --        in s * 3 + x
...
importTx net br tt tx = do
    ...
    --     $(logErrorS) "BlockLogic" $ "Transaction spends same output twice: " <> txHashToHex (txHash tx)
    --     throwError (DuplicatePrevOutput (txHashToHex (txHash tx)))
    -- when (iscb && not (confirmed br)) $ do
    --     $(logErrorS) "BlockLogic" $ "Attempting to import coinbase to the mempool: " <> txHashToHex (txHash tx)
    --     throwError (UnconfirmedCoinbase (txHashToHex (txHash tx)))
  --   us <-
  --       if iscb
  --           then return []
  --           else forM (txIn tx) $ \TxIn {prevOutput = op} -> uns op
  --   when (not (confirmed br) && sum (map unspentAmount us) < sum (map outValue (txOut tx))) $ do
  --       $(logErrorS) "BlockLogic" $ "Insufficient funds: " <> txHashToHex (txHash tx)
  --       throwError (InsufficientFunds (txHashToHex th))
  --   zipWithM_ (spendOutput net br (txHash tx)) [0 ..] us
  --   zipWithM_ (\i o -> newOutput net br (OutPoint (txHash tx) i) o) [0 ..] (txOut tx)
  --   rbf <- getrbf
  --   let t =
  --           Transaction
  --               { transactionBlock = br
  ...
  --                   Nothing -> do
  --                       $(logErrorS) "BlockLogic" $
  --                           "No spent or unspent output: " <> txHashToHex (outPointHash op) <> " " <>
  --                           fromString (show (outPointIndex op))
  ...
  --                           Nothing -> do
  --                               $(logErrorS) "BlockLogic" $
  --                                   "Could not unspend output: " <> txHashToHex (outPointHash op) <> " " <>
  --                                   fromString (show (outPointIndex op))
  --                               throwError (NoUnspent (cs (show op)))
  --                           Just u -> return u
  --   th = txHash tx
  ...
  --           , inputWitness = w
  --           }
  --   mkout o = StoreOutput {outputAmount = outValue o, outputScript = scriptOutput o, outputSpender = Nothing}

confirmTx :: (MonadLogger m, MonadError ImportException m) => Network -> TxData -> BlockRef -> Tx -> m ()
confirmTx net t br tx = do
    ...
    --         Left _ -> return ()
    --         Right a -> do
    --             deleteAddrTx a BlockTx {blockTxBlock = txDataBlock t, blockTxHash = txHash tx}
    --             insertAddrTx a BlockTx {blockTxBlock = br, blockTxHash = txHash tx}
...
getRecursiveTx :: (Monad m, MonadLogger m) => TxHash -> m [Transaction]
getRecursiveTx th = undefined
    -- getTxData th >>= \case
    --     Nothing -> return []
    --     Just d -> do
    --         sm <- getSpenders th
...
deleteTx net mo h = do
  ...
  --   getTxData h >>= \case
  --       Nothing -> do
  --           $(logErrorS) "BlockLogic" $ "Transaciton not found: " <> txHashToHex h
  --           throwError (TxNotFound (txHashToHex h))
  --       Just t
  --           | txDataDeleted t -> do
  --               $(logWarnS) "BlockLogic" $ "Transaction already deleted: " <> txHashToHex h
  --               return ()
  --           | mo && confirmed (txDataBlock t) -> do
  --               $(logErrorS) "BlockLogic" $ "Will not delete confirmed transaction: " <> txHashToHex h
  ...
  --       unless (confirmed (txDataBlock t)) $ deleteMempoolTx h (memRefTime (txDataBlock t))
  --       insertTx t {txDataDeleted = True}
  --       updateAddressCounts net (txDataAddresses t) (subtract 1)

insertDeletedMempoolTx :: (MonadLogger m, MonadError ImportException m) => Tx -> UnixTime -> m ()
insertDeletedMempoolTx tx w = do
    undefined
  --   us <- forM (txIn tx) $ \TxIn {prevOutput = op} -> getImportTx (outPointHash op) >>= getTxOutput (outPointIndex op)
  --   rbf <- getrbf
  --   let (d, _) =
  --           fromTransaction
  --               Transaction
  ...
  --                   , transactionTime = w
  --                   }
  --   $(logWarnS) "BlockLogic" $ "Inserting deleted mempool transaction: " <> txHashToHex (txHash tx)
  --   insertTx d
  -- where
  --   ws = map Just (txWitness tx) <> repeat Nothing
  --   getrbf
  ...
  --                   getTxData h >>= \case
  --                       Nothing -> do
  --                           $(logErrorS) "BlockLogic" $ "Transaction not found: " <> txHashToHex h
  --                           throwError (TxNotFound (txHashToHex h))
  --                       Just t
  --                           | confirmed (txDataBlock t) -> return False
  --                           | txDataRBF t -> return True
...
newOutput :: (MonadLogger m) => Network -> BlockRef -> OutPoint -> TxOut -> m ()
newOutput net br op to = do
  ...
  --           insertAddrTx a BlockTx {blockTxHash = outPointHash op, blockTxBlock = br}
...
delOutput :: (MonadLogger m, MonadError ImportException m) => Network -> OutPoint -> m ()
delOutput net op = do
    undefined
    -- t <- getImportTx (outPointHash op)
    ...
    --                 }
    --         deleteAddrTx a BlockTx {blockTxHash = outPointHash op, blockTxBlock = transactionBlock t}
    --         reduceBalance net (confirmed (transactionBlock t)) True a (outputAmount u)

getImportTx :: (MonadLogger m, MonadError ImportException m) => TxHash -> m Transaction
getImportTx th = undefined
    -- getTxData th >>= \case
    --     Nothing -> do
    --         $(logErrorS) "BlockLogic" $ "Tranasction not found: " <> txHashToHex th
    --         throwError $ TxNotFound (txHashToHex th)
    --     Just d
    --         | txDataDeleted d -> do
    --             $(logErrorS) "BlockLogic" $ "Transaction deleted: " <> txHashToHex th
...
spendOutput ::
       (MonadLogger m, MonadError ImportException m) => Network -> BlockRef -> TxHash -> Word32 -> Unspent -> m ()
spendOutput net br th ix u = do
    ...
    --     Left _ -> return ()
    --     Right a -> do
    --         reduceBalance net (confirmed (unspentBlock u)) False a (unspentAmount u)
    --         deleteAddrUnspent a u
    --         insertAddrTx a BlockTx {blockTxHash = th, blockTxBlock = br}
    -- deleteUnspent (unspentPoint u)

unspendOutput :: (MonadLogger m, MonadError ImportException m) => Network -> OutPoint -> m ()
unspendOutput net op = do
    ...
    --     Right a -> do
    --         insertAddrUnspent a u
    --         deleteAddrTx a BlockTx {blockTxHash = spenderHash s, blockTxBlock = transactionBlock x}
    --         increaseBalance (confirmed (unspentBlock u)) False a (outputAmount o)
...
reduceBalance ::
    ...
    -> Word64
    -> m ()
reduceBalance net c t a v = undefined
  --   getBalance a >>= \case
  --       Nothing -> do
  --           $(logErrorS) "BlockLogic" $ "Balance not found for address " <> addrText net a
  --           throwError (BalanceNotFound (addrText net a))
  --       Just b -> do
  --           when (v > amnt b) $ do
  --               $(logErrorS) "BlockLogic" $
  --                   "Insufficient " <> conf <> " balance: " <> addrText net a <> " (needs: " <> cs (show v) <> ", has: " <>
  --                   cs (show (amnt b)) <>
  --                   ")"
  --               throwError $
  --                   if c
