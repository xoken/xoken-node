...
module Network.Xoken.Node.Block
    ( blockStore
    ) where
...
import Network.Xoken.Node.Logic
...
--
-- instance (MonadResource m, MonadUnliftIO m) => StoreStream (ReaderT BlockRead m) where
...
blockStore :: (MonadUnliftIO m, MonadLoggerIO m) => BlockConfig -> Inbox BlockMessage -> m ()
blockStore cfg inbox = do
    $(logInfoS) "Block" "Initializing block store..."
    pb <- newTVarIO Nothing
    runReaderT (ini >> run) BlockRead {mySelf = inboxToMailbox inbox, myConfig = cfg, myPeer = pb}
  where
    ini = do
        ...
        --     Left e -> do
        --         $(logErrorS) "Block" $ "Could not initialize block store: " <> fromString (show e)
        --         throwIO e
    ...
    run =
        withAsync (pingMe (inboxToMailbox inbox)) . const . forever $ do
            ...
            receive inbox >>= \msg -> ReaderT $ \r -> runResourceT (runReaderT (processBlockMessage msg) r)
    ...
    getBestBlockDB >>= \case
        ...
        Just bb -> do
            ...
            chainGetBest ch >>= \cb -> do
                $(logDebugS) "Block" $
                    "Best chain block " <> blockHashToHex (headerHash (nodeHeader cb)) <> " at height " <>
                    cs (show (nodeHeight cb))
                let s = headerHash (nodeHeader cb) == bb
                $(logDebugS) "Block" $ "Synced: " <> cs (show s)
                return s

mempool :: (MonadUnliftIO m, MonadLoggerIO m) => Peer -> ReaderT BlockRead m ()
mempool p = MMempool `sendMessage` p

processBlock :: (MonadUnliftIO m, MonadLoggerIO m) => Peer -> Block -> ReaderT BlockRead m ()
processBlock p b = do
    $(logDebugS) "Block" ("Processing incoming block: " <> hex)
    void . runMaybeT $ do
        iss >>= \x ->
            unless x $ do
                $(logErrorS) "Block" $ "Cannot accept block " <> hex <> " from non-syncing peer"
                mzero
        n <- cbn
        upr
        ...
        --         $(logInfoS) "Block" $
        --             "Best block indexed: " <> blockHashToHex (headerHash (blockHeader b)) <> " at height " <>
        --             cs (show (nodeHeight n))
        --         atomically $ l (StoreBestBlock (headerHash (blockHeader b)))
        --         lift $ isSynced >>= \x -> when x (mempool p)
        --     Left e -> do
        --         $(logErrorS) "Block" $
        --             "Error importing block " <> fromString (show $ headerHash (blockHeader b)) <> ": " <>
        --             fromString (show e)
            ...
            Just s@Syncing {syncingHead = h} ->
                if nodeHeader h == blockHeader b
                    then resetPeer
                        ...
                        asks myPeer >>= atomically . (`writeTVar` Just s {syncingTime = now})
    iss =
        asks myPeer >>= readTVarIO >>= \case
            Just Syncing {syncingPeer = p'} -> return $ p == p'
            Nothing -> return False
    cbn =
        blockConfChain <$> asks myConfig >>= chainGetBlock (headerHash (blockHeader b)) >>= \case
            Nothing -> do
                killPeer (PeerMisbehaving "Sent unknown block") p
                $(logErrorS) "Block" ("Block " <> hex <> " rejected as header not found")
                mzero
            Just n -> return n

processNoBlocks :: (MonadUnliftIO m, MonadLoggerIO m) => Peer -> [BlockHash] -> ReaderT BlockRead m ()
processNoBlocks p _bs = do
    $(logErrorS) "Block" "We do not like peers that cannot find them blocks"
    killPeer (PeerMisbehaving "We do not like peers that cannot find them blocks") p

processTx :: (MonadUnliftIO m, MonadLoggerIO m) => Peer -> Tx -> BlockT m ()
processTx _p tx =
    isSynced >>= \case
        False -> $(logDebugS) "Block" $ "Ignoring incoming tx (not synced yet): " <> txHashToHex (txHash tx)
        True -> do
            ...
            --     Left e ->
            --         $(logWarnS) "Block" $
            --         "Error importing tx: " <> txHashToHex (txHash tx) <> ": " <> fromString (show e)
            --     Right True -> do
            --         l <- blockConfListener <$> asks myConfig
            --         $(logDebugS) "Block" $ "Received mempool tx: " <> txHashToHex (txHash tx)
            --         atomically $ l (StoreMempoolNew (txHash tx))
            --     Right False -> $(logDebugS) "Block" $ "Not importing mempool tx: " <> txHashToHex (txHash tx)

processOrphans :: (MonadResource m, MonadUnliftIO m, MonadLoggerIO m) => BlockT m ()
processOrphans = undefined
    -- isSynced >>= \case
    ...
    --         old <- runConduit $ getOldOrphansDB now .| sinkList
    --         case old of
    --             [] -> $(logDebugS) "Block" "No old orphan transactions to remove"
    --             _ -> do
    --                 $(logDebugS) "Block" $ "Removing " <> cs (show (length old)) <> "expired orphan transactions..."
    ...
    --             _ -> $(logDebugS) "Block" $ "Importing " <> cs (show (length orphans)) <> " orphan transactions"
    --         -- forM_ orphans $ runImport . uncurry (importOrphan net)
    --         $(logDebugS) "Block" "Finished importing orphans"

processTxs :: (MonadUnliftIO m, MonadLoggerIO m) => Peer -> [TxHash] -> ReaderT BlockRead m ()
processTxs p hs =
    isSynced >>= \case
        False -> $(logDebugS) "Block" "Ignoring incoming tx inv (not synced yet)"
        True -> do
            $(logDebugS) "Block" $ "Received " <> fromString (show (length hs)) <> " transaction inventory"
            xs <-
                fmap catMaybes . forM hs $ \h ->
                    runMaybeT $ do
                        ...
                        return (getTxHash h)
            unless (null xs) $ do
                $(logDebugS) "Block" $ "Requesting " <> fromString (show (length xs)) <> " new transactions"
                net <- blockConfNet <$> asks myConfig
                let inv =
                        if getSegWit net
                            then InvWitnessTx
...
processDisconnect :: (MonadUnliftIO m, MonadLoggerIO m) => Peer -> ReaderT BlockRead m ()
processDisconnect p =
    asks myPeer >>= readTVarIO >>= \case
        Nothing -> $(logDebugS) "Block" "Ignoring peer disconnection notification"
...
purgeMempool :: (MonadUnliftIO m, MonadLoggerIO m) => ReaderT BlockRead m ()
...
syncMe =
    void . runMaybeT $ do
        ...
        p <-
            gpr >>= \case
                Nothing -> do
                    $(logWarnS) "Block" "Not syncing blocks as no peers available"
                    mzero
                Just p -> return p
        ...
        c <- cbn
        when (end b d c) $ do
            $(logDebugS) "Block" $ "Already requested up to block height: " <> cs (show (nodeHeight b))
            mzero
        ns <- bls c b
        setPeer p (last ns)
        net <- blockConfNet <$> asks myConfig
        ...
        vs <- mapM (f inv) ns
        $(logInfoS) "Block" $ "Requesting " <> fromString (show (length vs)) <> " blocks"
  ...
  where
    ...
    dbn = do
        ...
        chainGetBlock bb ch >>= \case
            Nothing -> do
                $(logErrorS) "Block" $ "Block header not found for best block " <> blockHashToHex bb
    ...
    f inv BlockNode {nodeHeader = bh} = return $ InvVector inv (getBlockHash (headerHash bh))
    bls c b = do
    ...
    top c t = do
        ...
        if t == nodeHeight c
            then return c
                     ...
                     Nothing -> do
                         $(logErrorS) "Block" $
                             "Unknown header for ancestor of block " <> blockHashToHex (headerHash (nodeHeader c)) <>
                             " at height " <>
                             fromString (show t)
                         throwIO $ AncestorNotInChain t (headerHash (nodeHeader c))
    rev = do
        d <- headerHash . nodeHeader <$> dbn
        ch <- blockConfChain <$> asks myConfig
        chainBlockMain d ch >>= \m ->
            unless m $ do
                $(logErrorS) "Block" $ "Reverting best block " <> blockHashToHex d <> " as it is not in main chain..."
                resetPeer
                net <- asks (blockConfNet . myConfig)
                undefined
                -- lift (runImport (revertBlock net d)) >>= \case
                --     Left e -> do
                --         $(logErrorS) "Block" $ "Could not revert best block: " <> fromString (show e)
                --         throwIO e
                --     Right () -> rev
...
setPeer p b = do
    box <- asks myPeer
    now <- fromIntegral . systemSeconds <$> liftIO getSystemTime
    atomically . writeTVar box $ Just Syncing {syncingPeer = p, syncingHead = b, syncingTime = now}

processBlockMessage :: (MonadResource m, MonadUnliftIO m, MonadLoggerIO m) => BlockMessage -> BlockT m ()
...
processBlockMessage (BlockPing r) = processOrphans >> checkTime >> atomically (r ())
