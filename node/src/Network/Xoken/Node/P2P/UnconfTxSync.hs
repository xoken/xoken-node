...
commitEpochScriptHashOutputs conn epoch sh output = do
    ...
    resAddrOuts <- liftIO $ try $ write (Q.RqQuery $ Q.Query (qstrAddrOuts) parAddrOuts)
    case resAddrOuts of
        Right _ -> return ()
        Left (e :: SomeException) -> do
...
commitEpochScriptHashUnspentOutputs ::
       (HasLogger m, MonadIO m) => XCQLServerState k a b -> Bool -> Text -> (Text, Int32) -> m ()
...
--        XCQLServerState k a b -> (SM.Map TxHash EV.Event) -> Logger -> Network -> OutPoint -> Int -> IO (Maybe Text)
