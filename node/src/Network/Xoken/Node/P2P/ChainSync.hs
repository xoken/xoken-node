...
markBestBlock :: (HasLogger m, MonadIO m) => Text -> Int32 -> XCQLServerState k a b -> m ()
...
getBlockLocator :: (HasLogger m, MonadIO m) => XCQLServerState k a b -> Network -> m ([BlockHash])
...
fetchBestBlock :: (HasLogger m, MonadIO m) => XCQLServerState k a b -> Network -> m ((BlockHash, Int32))
...
fetchMatchBlockOffset :: (HasLogger m, MonadIO m) => XCQLServerState k a b -> Network -> Text -> m (Maybe (Text, Int32))
...
updateChainWork :: (HasLogger m, MonadIO m) => [(Int32, BlockHeaderCount)] -> XCQLServerState k a b -> m ()
