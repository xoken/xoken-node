...
new size = do
    hlist <- mapM (\x -> H.new) [1 .. size]
    hlock <- mapM (\x -> newMVar True) [1 .. size]
    return $ TSHashTable hlist hlock size

insert :: (Eq k, Hashable k) => TSHashTable k v -> k -> v -> IO ()
insert tsh k v = do
    let index = (hash k) `mod` (fromIntegral $ size tsh)
    liftIO $ takeMVar ((hashTableLocks tsh) !! index)
    H.insert ((hashTableList tsh) !! index) k v
    liftIO $ putMVar ((hashTableLocks tsh) !! index) True
    return ()
...
delete tsh k = do
    let index = (hash k) `mod` (fromIntegral $ size tsh)
    liftIO $ takeMVar ((hashTableLocks tsh) !! index)
    H.delete ((hashTableList tsh) !! index) k
    liftIO $ putMVar ((hashTableLocks tsh) !! index) True
    return ()
...
lookup tsh k = do
    ...
    liftIO $ takeMVar ((hashTableLocks tsh) !! index)
    liftIO $ putMVar ((hashTableLocks tsh) !! index) True
    H.lookup ((hashTableList tsh) !! index) k
...
mapM_ f tsh = do
    traverse
        (\(ht, htl) -> do
             liftIO $ takeMVar htl
             H.mapM_ f ht
             liftIO $ putMVar htl True)
        (L.zip (hashTableList tsh) (hashTableLocks tsh))
    return ()

toList :: (Eq k, Hashable k) => TSHashTable k v -> IO [(k, v)]
toList tsh = fmap L.concat $ traverse (H.toList) (hashTableList tsh)
