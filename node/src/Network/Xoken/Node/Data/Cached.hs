...
module Network.Xoken.Node.Data.Cached where
...
import UnliftIO
import Xoken
-- newLayeredDB :: MonadUnliftIO m => BlockDB -> m LayeredDB
-- newLayeredDB blocks = return LayeredDB {layeredDB = blocks}
--   where
...
--
-- getUnspentsC :: (MonadResource m, MonadIO m) => LayeredDB -> ConduitT i Unspent m ()
-- getUnspentsC LayeredDB {layeredDB = db} = getUnspentsDB db
--
-- getMempoolC :: (MonadResource m, MonadUnliftIO m) => LayeredDB -> ConduitT i (UnixTime, TxHash) m ()
-- -- getMempoolC LayeredDB {layeredCache = Just db} = getMempoolDB db
-- getMempoolC LayeredDB {layeredDB = db} = getMempoolDB db
--
-- getOrphansC :: (MonadUnliftIO m, MonadResource m) => LayeredDB -> ConduitT i (UnixTime, Tx) m ()
-- getOrphansC LayeredDB {layeredDB = db} = getOrphansDB db
--
-- getAddressBalancesC :: (MonadUnliftIO m, MonadResource m) => LayeredDB -> ConduitT i Balance m ()
-- getAddressBalancesC LayeredDB {layeredDB = db} = getAddressBalancesDB db
--
-- getAddressUnspentsC ::
--        (MonadUnliftIO m, MonadResource m) => Address -> Maybe BlockRef -> LayeredDB -> ConduitT i Unspent m ()
-- getAddressUnspentsC addr mbr LayeredDB {layeredDB = db} = getAddressUnspentsDB addr mbr db
--
-- getAddressTxsC ::
--        (MonadUnliftIO m, MonadResource m) => Address -> Maybe BlockRef -> LayeredDB -> ConduitT i BlockTx m ()
-- getAddressTxsC addr mbr LayeredDB {layeredDB = db} = getAddressTxsDB addr mbr db
--
-- bulkCopy :: (Serialize k, MonadUnliftIO m) => ReadOptions -> DB -> DB -> k -> m ()
...
--
-- instance (MonadUnliftIO m, MonadResource m) => StoreStream (ReaderT LayeredDB m) where
--     getMempool = do
--         c <- R.ask
--         getMempoolC c
--     getOrphans = do
