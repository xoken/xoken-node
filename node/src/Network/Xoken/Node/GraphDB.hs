...
insertProtocolWithBlockInfo name properties BlockPInfo {..} = do
    res <- LE.try $ queryP cypher params
    liftIO $ print $ "insertProtocolWithBlockInfo query: " <> cypher
    case res of
        Left (e :: SomeException) -> do
  ...
  where
    cypher =
        ...
        props <>
        "  }) " <>
        " MERGE (b: block { hash: {hash}, height: {height}, timestamp: {timestamp}, day: {day }, month: {month}, year: {year}, hour: {hour}, absoluteHour: {absoluteHour} }) WITH a, b" <>
