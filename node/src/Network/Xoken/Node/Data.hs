...
blockRefPairs BlockRef {blockRefHeight = h, blockRefPos = p} = ["height" .= h, "position" .= p]
...
blockTxPairs btx = ["txid" .= blockTxHash btx, "block" .= blockTxBlock btx]
...
instance BinSerial Balance where
    binSerial net Balance { balanceAddress = a
        ...
        putWord64be t
    binDeserial net =
        Balance <$> binDeserial net <*> getWord64be <*> getWord64be <*> getWord64be <*> getWord64be <*> getWord64be
...
instance Serialize Unspent where
    ...
    get = Unspent <$> get <*> get <*> get <*> (getShortByteString =<< get)

unspentPairs :: A.KeyValue kv => Network -> Unspent -> [kv]
unspentPairs net u =
    [ "address" .= eitherToMaybe (addrToJSON net <$> scriptToAddressBS (B.Short.fromShort (unspentScript u)))
...
instance BinSerial Unspent where
    binSerial net Unspent {unspentBlock = b, unspentPoint = p, unspentAmount = v, unspentScript = s} = do
        binSerial net b
        put p
        putWord64be v
        put s
    binDeserial net = Unspent <$> binDeserial net <*> get <*> getWord64be <*> get
...
spenderPairs n = ["txid" .= spenderHash n, "input" .= spenderIndex n]
...
outputPairs net d =
    [ "address" .= eitherToMaybe (addrToJSON net <$> scriptToAddressBS (outputScript d))
...
toInput i Nothing w =
    StoreCoinbase
        {inputPoint = prevOutput i, inputSequence = txInSequence i, inputSigScript = scriptInput i, inputWitness = w}
toInput i (Just p) w =
    StoreInput
...
toOutput o s = StoreOutput {outputAmount = outValue o, outputScript = scriptOutput o, outputSpender = s}
...
instance BinSerial TxData where
    binSerial _ TxData { txDataBlock = br
                       , txData = tx
                       , txDataPrevs = dp
                       , txDataDeleted = dd
                       , txDataRBF = dr
...
fromTransaction t = (d, sm)
  where
    ...
    f n StoreInput {inputPkScript = s, inputAmount = v} = Just (n, Prev {prevScript = s, prevAmount = v})
...
transactionData t =
  ...
  where
    ...
    i StoreInput {inputPoint = p, inputSequence = q, inputSigScript = s} =
        TxIn {prevOutput = p, scriptInput = s, txInSequence = q}
    o StoreOutput {outputAmount = v, outputScript = s} = TxOut {outValue = v, scriptOutput = s}
...
transactionPairs net dtx =
    ...
    , "fee" .=
      if all isCoinbase (transactionInputs dtx)
          then 0
          else sum (map inputAmount (transactionInputs dtx)) - sum (map outputAmount (transactionOutputs dtx))
...
instance BinSerial PeerInformation where
    binSerial _ PeerInformation {peerUserAgent = u, peerAddress = a, peerVersion = v, peerServices = s, peerRelay = b} = do
        putWord32be v
        put b
        put u
...
xPubBalPairs net XPubBal {xPubBalPath = p, xPubBal = b} = ["path" .= p, "balance" .= balanceToJSON net b]
...
xPubUnspentPairs net XPubUnspent {xPubUnspentPath = p, xPubUnspent = u} =
    ["path" .= p, "unspent" .= unspentToJSON net u]
...
xPubSummaryPairs net XPubSummary { xPubSummaryConfirmed = c
    ...
    [ "balance" .= object ["confirmed" .= c, "unconfirmed" .= z, "received" .= r, "utxo" .= u]
...
healthCheckPairs h =
    [ "headers" .= object ["hash" .= healthHeaderBest h, "height" .= healthHeaderHeight h]
    , "blocks" .= object ["hash" .= healthBlockBest h, "height" .= healthBlockHeight h]
    , "peers" .= healthPeers h
    , "net" .= healthNetwork h
    , "ok" .= healthOK h
...
balanceToBalVal Balance { balanceAddress = a
    ...
    (a, BalVal {balValAmount = v, balValZero = z, balValUnspentCount = u, balValTxCount = t, balValTotalReceived = r})

-- | Default balance for an address.
instance Default BalVal where
    def = BalVal {balValAmount = 0, balValZero = 0, balValUnspentCount = 0, balValTxCount = 0, balValTotalReceived = 0}
...
unspentToUnspentVal Unspent {unspentBlock = b, unspentPoint = p, unspentAmount = v, unspentScript = s} =
    (p, UnspentVal {unspentValBlock = b, unspentValAmount = v, unspentValScript = s})

unspentValToUnspent :: OutPoint -> UnspentVal -> Unspent
unspentValToUnspent p UnspentVal {unspentValBlock = b, unspentValAmount = v, unspentValScript = s} =
    Unspent {unspentBlock = b, unspentPoint = p, unspentAmount = v, unspentScript = s}
